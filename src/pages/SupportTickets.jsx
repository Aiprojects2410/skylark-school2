...
import { AlertCircle, CheckCircle2, Clock3, MessageSquare, RefreshCw, Search, Ticket, XCircle, Bot } from 'lucide-react'
...
export default function SupportTickets() {
  ...
  const stats = useMemo(() => ({
    ...
    ai: tickets.filter(t => t.ai_status === 'investigating').length,
  }), [tickets])
  ...
  function handleAiUpdated(updated) {
    setSelected(v => v ? { ...v, ...updated } : v)
    setTickets(v => v.map(t => t.id === updated.id ? { ...t, ...updated } : t))
  }

  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm font-semibold text-brand">Administration</p><h1 className="text-2xl font-black text-slate-900 dark:text-white">Support Tickets</h1><p className="mt-1 text-sm text-slate-500">Review, respond to, investigate and resolve issues reported from any portal.</p></div><button onClick={load} className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold dark:border-slate-700 dark:bg-slate-900"><RefreshCw size={16} /> Refresh</button></div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        {[[Ticket, 'Total', stats.total, 'slate'], [AlertCircle, 'Open', stats.open, 'blue'], [Clock3, 'In Progress', stats.progress, 'amber'], [Bot, 'AI Investigating', stats.ai, 'violet'], [CheckCircle2, 'Resolved', stats.resolved, 'green'], [XCircle, 'High Priority', stats.high, 'red']].map(([Icon, label, value, tone]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-500">{label}</span><Icon size={17} /></div><p className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{value}</p><Badge tone={tone}>{label}</Badge></div>)}
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800"><div className="flex items-center gap-2 rounded-xl bg-slate-100 px-3 py-2 sm:w-80 dark:bg-slate-800"><Search size={16} className="text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search ticket, user or issue..." className="w-full bg-transparent text-sm outline-none" /></div><span className="text-xs text-slate-500">{filtered.length} ticket{filtered.length === 1 ? '' : 's'}</span></div>
        {loading ? <div className="p-8 text-center text-sm text-slate-500">Loading tickets…</div> : filtered.length === 0 ? <div className="p-10 text-center text-sm text-slate-500">No tickets found.</div> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-950"><tr><th className="px-4 py-3">Ticket</th><th className="px-4 py-3">User</th><th className="px-4 py-3">Issue</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">AI</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Date</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{filtered.map(t => <tr key={t.id} onClick={() => openTicket(t)} className="cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/60"><td className="px-4 py-3 font-black text-brand">#{t.ticket_number}</td><td className="px-4 py-3"><p className="font-semibold">{t.profiles?.full_name || 'User'}</p><p className="text-xs text-slate-400 capitalize">{t.profiles?.role || ''}</p></td><td className="max-w-xs px-4 py-3"><p className="truncate font-semibold">{t.subject || 'Issue reported'}</p><p className="truncate text-xs text-slate-400">{t.description}</p></td><td className="px-4 py-3"><Badge tone={t.priority === 'high' ? 'red' : t.priority === 'medium' ? 'amber' : 'slate'}>{t.priority}</Badge></td><td className="px-4 py-3">{t.ai_status === 'completed' ? <Badge tone="violet">Investigated</Badge> : t.ai_status === 'investigating' ? <Badge tone="amber">Running</Badge> : <span className="text-xs text-slate-400">Not run</span>}</td><td className="px-4 py-3"><Badge tone={t.status === 'open' ? 'blue' : t.status === 'in_progress' ? 'amber' : 'green'}>{t.status.replace('_', ' ')}</Badge></td><td className="whitespace-nowrap px-4 py-3 text-xs text-slate-500">{new Date(t.created_at).toLocaleString()}</td></tr>)}</tbody></table></div>}
      </div>
