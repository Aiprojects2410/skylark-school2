...
export default function Teachers() {
  ...
  const handleLogin = async (teacher) => {
    ...
    try {
      ...
      notify(result?.message || 'Teacher login created successfully.')
      await load()
    } catch (e) { notify(e.message) }
            ...
            <tbody>
              {teachers.map(t => (
                <tr key={t.id}>
                  ...
                  <td>{t.phone || '—'}</td>
                  <td>{t.email || '—'}</td>
                  <td>{t.auth_user_id ? <span className="text-xs font-semibold text-emerald-600">Created</span> : <button disabled={busyId === t.id} className="text-xs font-semibold text-brand hover:underline disabled:opacity-50" onClick={() => handleLogin(t)}>{busyId === t.id ? 'Creating…' : 'Create login'}</button>}</td>
