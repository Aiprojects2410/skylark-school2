...
export default function Students() {
  ...
  // Teachers get a read-only directory — adding, deleting, and creating logins are admin-only actions.
  ...
  // Group by class, then section, so the directory reads as a real school roster.
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700">
            <Search size={16} /><input value={query} onChange={e => setQuery(e.target.value)} className="bg-transparent outline-none" placeholder="Search students" />
          </label>
        </div>
      </div>

      {loading ? <p className="text-sm text-slate-400">Loading students…</p> : grouped.length === 0 ? (
        ...
        <div className="space-y-6">
          {grouped.map(([className, sections]) => (
            <section key={className} className="card overflow-hidden !p-0">
              <div className="border-b bg-slate-50 px-5 py-3 dark:border-slate-800 dark:bg-slate-800/50">
                <p className="font-bold text-ink dark:text-white">{className}</p>
              </div>
              {Object.entries(sections).sort(([a], [b]) => a.localeCompare(b)).map(([sectionName, list]) => (
                <div key={sectionName} className="overflow-x-auto">
                  ...
                  <table>
                    <thead><tr><th>Student</th><th>Admission no.</th><th>Guardian</th><th>Status</th><th className="text-right">Actions</th></tr></thead>
                    <tbody>
                      {list.map(s => (
                        <tr key={s.id}>
                          ...
                          <td><p>{s.parent_name || s.father_name || '—'}</p><p className="text-xs text-slate-400">{s.parent_phone}</p></td>
                          <td><Badge>{s.status || 'active'}</Badge></td>
                          <td className="text-right">
                            {canManage ? (
                              <>
                                <button onClick={() => setModal(s)} className="mr-3 text-sm font-semibold text-brand">Edit</button>
                                <button onClick={() => createLogin(s)} className="mr-3 text-sm font-semibold text-emerald-600">{s.auth_user_id ? 'Reset login' : 'Create login'}</button>
                                <button onClick={() => remove(s)} className="text-sm font-semibold text-rose-600">Delete</button>
                              </>
