...
export default function Timetable() {
  ...
  const load = async () => {
    try {
      const [t, teachersData, classesResult] = await Promise.all([
        getTimetableSlots(), getTeachers(), supabase ? supabase.from('classes').select('id,name').order('name') : Promise.resolve({ data: [] })
      ])
  ...
  const remove = async id => { try { await deleteTimetableSlot(id); await load() } catch (e) { notify(e.message) } }

  return (
    <>
      <PageHeader eyebrow="SCHEDULE" title="Class Timetable" subtitle="Daily period schedule, subject allocations, and teacher assignments." action={<button className="btn-primary" onClick={() => setShowModal(true)}>+ Add Timetable</button>} />
      <div className="card overflow-x-auto">
        {loading ? <p className="p-5 text-sm text-slate-400">Loading timetable…</p> : <table>
          <thead><tr><th>Day</th><th>Time / Period</th><th>Subject</th><th>Teacher</th><th>Class</th><th>Actions</th></tr></thead>
          <tbody>
            {slots.map(s => <tr key={s.id}><td>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][s.day_of_week]}</td><td className="font-semibold text-brand">{s.start_time?.slice(0,5)} - {s.end_time?.slice(0,5)}</td><td>{s.subject}</td><td>{s.teacher_name || '—'}</td><td>{s.class_name ? `${s.class_name}${s.section_name ? ` ${s.section_name}` : ''}` : '—'}</td><td><button className="text-xs font-semibold text-rose-600 hover:underline" onClick={() => remove(s.id)}>Delete</button></td></tr>)}
            {!slots.length && legacy.map((s, i) => <tr key={`legacy-${i}`}><td>—</td><td className="font-semibold text-brand">{s.period}</td><td>{s.subject}</td><td>{s.teacher_name}</td><td>—</td><td className="text-xs text-slate-400">Legacy</td></tr>)}
          </tbody>
        </table>}
      </div>
      {showModal && <Modal title="Add Timetable Entry" onClose={() => setShowModal(false)}><form onSubmit={submit} className="space-y-4 p-5">
