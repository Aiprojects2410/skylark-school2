...
export default function Scanner() {
  ...
  async function handleCode(token) {
    ...
    try {
      ...
      setLog(l => [{ token, ...outcome, at: new Date() }, ...l].slice(0, 20))
    } catch (e) { notify(e.message) } finally { setTimeout(() => setBusy(false), 1200) }
  }

  const { videoRef, cameraOn, error, start, stop } = useQrCamera(handleCode)

  function submitManual(e) { e.preventDefault(); if (manualCode.trim()) { handleCode(manualCode.trim()); setManualCode('') } }
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fc]">
      <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-6 py-4">
        ...
        <div><p className="text-sm font-bold text-ink">Skylark Scanner</p><p className="text-xs text-slate-400">Standalone attendance kiosk</p></div>
      ...
      <div className="mx-auto max-w-5xl p-6">
        <div className="mb-6">
          ...
          <p className="section-subtitle">Scan an identity card's QR code to instantly log attendance.</p>
        </div>
        ...
        <section className="card">
          ...
          <div className="mt-4 space-y-2">
            ...
            {log.map((entry, i) => {
              const copy = RESULT_COPY[entry.result]
