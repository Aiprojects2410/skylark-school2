    ...
    <div className="min-h-screen bg-[#f6f8fc] text-slate-700 dark:bg-slate-950 dark:text-slate-200">
      ...
      <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
        <div className="flex items-center gap-3 px-6 py-7">
          ...
          <div><p className="font-bold leading-none text-white">Skylark</p><p className="mt-1 text-xs text-blue-200">SCHOOL ERP</p></div>
        ...
        <header className="flex h-[76px] items-center gap-4 border-b border-slate-200/80 bg-white px-5 sm:px-8 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(v => !v)} className="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Notifications"><Bell size={20} /><i className="absolute right-2 top-2 h-2 w-2 rounded-full bg-rose-500" /></button>
              {notifOpen && <div className="absolute right-0 z-30 mt-2 w-72 rounded-xl border border-slate-100 bg-white p-3 shadow-xl dark:border-slate-800 dark:bg-slate-900"><p className="px-2 pb-2 text-sm font-bold text-ink dark:text-white">Notifications</p><div className="space-y-1 text-sm"><p className="rounded-lg px-2 py-2 hover:bg-slate-50 dark:hover:bg-slate-800">New notice: Term 1 examination timetable</p><p className="rounded-lg px-2 py-2 hover:bg-slate-50 dark:hover:bg-slate-800">3 leave requests awaiting approval</p></div></div>}
            </div>
            <div className="relative">
              <button onClick={() => setMenuOpen(v => !v)} className="flex items-center gap-2"><Avatar name={profile?.full_name} /><div className="hidden text-left sm:block"><p className="text-sm font-bold text-ink dark:text-white">{profile?.full_name || 'Loading…'}</p><p className="text-xs capitalize text-slate-500 dark:text-slate-400">{role?.replace('_', ' ') || ''}</p></div></button>
              {menuOpen && <div className="absolute right-0 z-30 mt-2 w-48 rounded-xl border border-slate-100 bg-white p-2 shadow-xl dark:border-slate-800 dark:bg-slate-900"><button onClick={() => { setMenuOpen(false); navigate('/settings') }} className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-800">Settings</button><button onClick={handleLogout} className="block w-full rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40">Sign out</button></div>}
            </div>
          </div>
        </header>
